...
@endif
      ...
      <section id="movie-selection">
        ...
        {{-- <a     class="balcony_btn "href="{{route('book_balcony')}}"> Balcony Booking</a> --}}
      </div>
        <h2>Main Hall seats </h2>
      </section>
...
{{--                 
               ...
               // $seat= \App\Models\Seat::all()->where('x_factor',$x)->where('y_factor',$y)->all();
                ...
                <div class="seat 
                  ...
                  {{"selected"}}
                   @elseif(DB::table('seats')->where('x_factor',$x)->where('y_factor',$y)->value('status_g'))
                  {{"occupied"}}
                  @endif
          ...
          <div id="seating-key">
            ...
            <div>Vacant   NO--> {{$count-count(DB::table('bookings')->where('level','0')->where('status','1')->get())-count(DB::table('bookings')->where('level','0')->where('status','0')->get())}}</div>
            <div class="seat selected"><div></div></div>
            <div>Paid   NO--> {{count(DB::table('bookings')->where('level','0')->where('status','1')->get())}}</div>
            <div class="seat occupied"><div></div></div>
            
            <div>Occupied   NO--> {{count(DB::table('bookings')->where('level','0')->where('status','0')->get())}}</div>
          </div>
